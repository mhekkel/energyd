	...
	while (token == token_t::Undef)
	{
		...
		case state_t::Escape:
			switch (ch)
			{
				...
				default:
					throw zeep::exception("Invalid escape sequence in json (\\" + ch + ')');
			}
...
// 			: m_impl(imp), m_lookahead(0), m_data_source(source), m_version(1.0f), m_encoding(enc_UTF8), m_external_subset(true), m_external_dtd(false)
// 		{
// 			swap_state();
// 		}
